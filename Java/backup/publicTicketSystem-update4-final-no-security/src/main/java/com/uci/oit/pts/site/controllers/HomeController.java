...
public class HomeController {
	...
	@Inject
	public HomeController(TicketsServiceInterface ticketsService, LoginServiceInterface LoginService){
	...
	public String editTicket(@RequestParam int id, @RequestParam(required=false) String status, @RequestParam(required=false) String assignedTo, 
		...
		if(!note.isEmpty()){
			resultNotesAdding= TicketsService.addNote(note, id); 
		}
	...
	public String addTicket(ProblemForm problem, Model model){
		...
		Set<ConstraintViolation<ProblemForm>> res= TicketsService.createTicket(problem);
		String response="";
		if (res.size() <= 0){
		...
		else
		{
				...
				model.addAttribute("Violations", res);
				model.addAttribute("pageName", "add");
				return "Add";
		}
